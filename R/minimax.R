...
balance_minimax = function(X, W, zeta, solver = c("ECOS", "SCS"), verbose = TRUE) {
    ...
    objective = (1 - zeta) * sum(gg[1:nobs]^2) + zeta / nobs^2 * sum(gg[nobs + 1:2]^2)
    contraints = list(
        sum(gg[1:nobs]) == 0,
        t(X) %*% gg[1:nobs] <= nobs * gg[nobs + 1],
        -t(X) %*% gg[1:nobs] <= nobs * gg[nobs + 1],
        sum(W * gg[1:nobs]) == nobs,
        t(X) %*% (W * gg[1:nobs]) <= colSums(X) + nobs * gg[nobs + 2],
        - t(X) %*% (W * gg[1:nobs]) <= - colSums(X) + nobs * gg[nobs + 2]
    )
